...
class BoreholeSelector
{
	...
	updateData()
	{
		...
		this.renderer.renderViewport({ layers: 1 << 10, scene: this.scene, camera: this.camera, enableIdShader: true, useComposer: false }, { left: 0, bottom: 0, width: 1, height: 1 });
	...
	#updateSelectedIndicesGPU(left, top, width, height)
	{
		...
		for (let y = top; y < top + height; y += 1)
		{
			for (let x = left; x < left + width; x += 1)
			{
				...
				const selectedInstanceIndex = (this.pixelBuffer[i] << 16) + (this.pixelBuffer[i + 1] << 8) + this.pixelBuffer[i + 2] - 1;
	...
	#updateSelectionBox(normalizedX, normalizedY)
	{
		...
		for (let i = 0; i < highlightAttribute.count; i++)
		{
			...
			{
				const selectedInstanceId = this.scene.boreholes.info.id[i];
				this.selectedBoreholeIds.push(selectedInstanceId);
			}
		}
	}
}
